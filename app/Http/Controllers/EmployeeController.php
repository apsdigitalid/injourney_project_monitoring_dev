...
use App\DataTables\RmDataTable;
...
class EmployeeController extends AccountBaseController
{
    ...
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            abort_403(!in_array('employees', $this->user->modules) &&  !in_array('resource', $this->user->modules));
            return $next($request);
        });
    }
    ...
    public function index(EmployeesDataTable $dataTable)
    {
        ...
        abort_403(!in_array($viewPermission, ['all', 'added', 'owned', 'both']) || user()->role[0]->role_id != 1);
    ...
    public function store(StoreRequest $request)
    {
        ...
        if(user()->role[0]->role_id != 1){
            return Reply::successWithData(__('messages.employeeAdded'), ['redirectUrl' => route('resource.index')]);
        }
        else{
    ...
    public function rscMgt(RmDataTable $dataTable)
    {
        
        $this->pageTitle = 'app.menu.rscManagement';
        $viewPermission = user()->permission('view_employees');
        ...
        if (!request()->ajax()) {
            ...
            $this->designations = Designation::allDesignations();
            $this->totalEmployees = count($this->employees);
            $this->roles = Role::where('name', '<>', 'client')->orderBy('id', 'asc')->get();
        }
        return $dataTable->render('employees.rscMgt', $this->data);
    }

    public function rscMgt_show($id)
    {
        $this->employee = User::with(['employeeDetail', 'employeeDetail.designation', 'employeeDetail.department', 'leaveTypes', 'country'])->withoutGlobalScope('active')->with('employee')->withCount('member', 'agents', 'openTasks')->findOrFail($id);
        ...
        foreach($project as $k=>$v){
            $data = Project::get_task_status($v->id);
            $temp_data[] = array(
                'id'  => $v->id,
        ...
        $this->project = $temp_data;
        $this->project1 = json_encode($temp_data);
        ...
        case 'permissions':
            ...
            break;
        default:
            $this->view = 'employees.ajax.performance';
            break;
        }
        ...
        // return view('employees.show', $this->data);
        return view('employees.rscPerformance', $this->data);
    }
