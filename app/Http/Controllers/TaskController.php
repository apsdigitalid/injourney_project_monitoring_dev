...
class TaskController extends AccountBaseController
{
    ...
    public function changeStatus(Request $request)
    {
        ...
        // $task1 = Task::findOrFail($taskId);
        $taskBoardColumn = TaskboardColumn::where('slug', $status)->first();
        // $task1->board_column_id = $taskBoardColumn->id;

        if ($taskBoardColumn->slug == 'completed') {
            // $task1->board_column_id = $taskBoardColumn->id;
            // $task1->completed_on = Carbon::now()->format('Y-m-d');
            // $task1->save();
        ...
        if ($task->project_id != null) {
            ...
            if ($task->project->calculate_task_progress == 'true') {
                ...
                $this->calculateProjectProgress($task->project_id, $task->weight);
            }
        }
