    ...
    public function overviewDashboard()
    {
        ...
        $this->counts = DB::table('users')
            ->select(
                ...
                DB::raw('(select count(projects.id) from `projects` WHERE deadline < DATE(NOW())) as totalProjectOverDue'),
                DB::raw('(select count(projects.id) from `projects` WHERE status="finished") as finished'),
                DB::raw('(select count(projects.id) from `projects` WHERE status="in progress") as in_progress'),
                DB::raw('(select count(projects.id) from `projects` WHERE status="not started") as not_started'),
                DB::raw('(select count(projects.id) from `projects` WHERE status="on hold") as on_hold'),
                DB::raw('(select count(projects.id) from `projects` WHERE status="canceled") as canceled'),
                DB::raw('(select COUNT(id) FROM `project_milestones` WHERE status="complete") as completeMilestones'),
                DB::raw('(select COUNT(id) FROM `project_milestones` WHERE status="incomplete") as in_completeMilestones'),
