...
class Project extends BaseModel
{
    ...
    public static function get_total_task($id){
        $task = DB::table('projects')
                ->selectRaw('projects.project_name, COUNT(tasks.project_id) as count_task')
                ->leftJoin('tasks', 'tasks.project_id', '=', 'projects.id')
                ->whereIn('projects.id', explode(',', $id))
                ->groupBy('projects.id')
    ...
    public static function get_project($id){
        $projects = DB::table('project_members as pm')
                ->selectRaw('p.project_name, p.start_date, p.deadline, p.status, p.id')
    ...
    public static function get_task_status($id){
        // SELECT	
        //     COUNT(t.status) AS num,
        //     t.status
        // FROM	tasks AS t
        // WHERE t.project_id = 1
        // GROUP BY t.status
        $tasks = DB::table('tasks as t')
                ->selectRaw('COUNT(t.status) AS num, t.status')
