 ...
 * @method static \Illuminate\Database\Eloquent\Builder|Task whereWeight($value)
...
class Task extends BaseModel
{
    ...
    public static function get_detail_task($id, $user_id){
        $select ='';
        ...
        if($role->name == 'admin' || $role->name == 'client'){
            $select = '(SELECT COUNT(tasks.id) AS complete FROM tasks WHERE board_column_id="2" AND tasks.project_id = '.$id.') AS complete, 
                (SELECT COUNT(tasks.id) AS incomplete FROM tasks WHERE board_column_id="1" AND tasks.project_id = '.$id.') AS incomplete';
        }
        elseif($role->name == 'employee'){
            $select = '(SELECT COUNT(tasks.id) AS complete FROM tasks LEFT JOIN project_members AS pm ON pm.project_id = tasks.project_id WHERE board_column_id="2" AND tasks.project_id = "'.$id.'"  AND pm.user_id='.$user_id.' ) AS complete, 
                (SELECT COUNT(tasks.id) AS incomplete FROM tasks LEFT JOIN project_members AS pm ON pm.project_id = tasks.project_id WHERE board_column_id="1" AND tasks.project_id = "'.$id.'" AND pm.user_id='.$user_id.') AS incomplete';
        }
        $task = DB::table('tasks')
                ->selectRaw($select)
